...
const server = new Server({ name: "exergynet-mcp-server", version: "0.1.1" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        { name: "exergynet_estimate_gate", description: "Calculate exergy-gate profitability.", inputSchema: { type: "object", required: ["externalComputeCostUsd", "settlementCostUsd"], properties: { externalComputeCostUsd: { type: "number" }, settlementCostUsd: { type: "number" }, riskMarginUsd: { type: "number" } } } }
    ]
}));
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (request.params.name) {
        ...
        case "exergynet_estimate_gate": {
            const extCost = Number(args.externalComputeCostUsd);
            const setCost = Number(args.settlementCostUsd) + (Number(args.riskMarginUsd) || 0);
            return { content: [{ type: "text", text: extCost > setCost ? `ACTION: USE_EXERGYNET (Margin: ${extCost - setCost})` : "REJECT: Below threshold." }] };
        }
